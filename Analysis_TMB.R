...
library(TMB)
#dyn.load("LeesApproxTMB.dll")
#source("../R/LeesApproxTMB.R")
...
binWidthvec <-  rev(c(2, 5, 10))

SimRcpp <- SimTMB <- lapply(1:length(Stocks), function(x) lapply(1:length(ngtgVec), function(y) lapply(1:length(binWidthvec), list)))
...
for (st in 1:length(Stocks)) {
  ...
  for (bw in binWidthvec) {
    ...
    nout <- length(ngtgVec)
    Elapse <- ElapseTMB <- NLL <- rep(NA, nout)
    storeLens <- list()
    for (x in seq_along(ngtgVec)) {
      # Simulate length data with Lee's approx method

      print(paste0("bw = ", bw, ", ngtg = ", ngtgVec[x]))
      St <- Sys.time()
      Sim <- SimRcpp[[st]][[which(binWidthvec == bw)]][[x]] <- LeesApprox(FVec=annualF, ngtg=ngtgVec[x], maxsd, binwidth=bw, M,
                                                                          Linf, K, t0,  LFS, L5, Vmaxlen, LinfCV, maxage)
      Elapse[x] <- Sys.time() - St
      print(Sys.time() - St)

      St <- Sys.time()
      SimTMB[[st]][[which(binWidthvec == bw)]][[x]] <- LeesApproxTMB(FVec=annualF, ngtg=ngtgVec[x], maxsd, binwidth=bw, M,
                                                               Linf, K, t0,  LFS, L5, Vmaxlen, LinfCV, maxage)
      ElapseTMB[x] <- Sys.time() - St
      print(Sys.time() - St)
    ...
    store[[count]] <- data.frame(Elapse=Elapse, ElapseTMB = ElapseTMB, binWidth=bw, Stock=st,
